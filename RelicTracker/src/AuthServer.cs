...
public class AuthServer
{
    ...
    private async Task<bool> ExchangeCode(string code)
    {
        try
        {
            ...
            {
                Log.Warn("[RelicTracker] Auth response missing user or token");
                return false;
            }
            ...
            Log.Info($"[RelicTracker] Authenticated as user {user}");
            return true;
        }
        catch (Exception ex)
        {
            Log.Warn($"[RelicTracker] Auth code exchange failed: {ex.Message}");
            return false;
        }
    }
    ...
    private static void OpenBrowser(string url)
    {
        ...
        {
            Log.Warn($"[RelicTracker] Failed to open browser: {ex.Message}");
            Log.Info($"[RelicTracker] Please open manually: {url}");
        }
    }
