...
{
    private const string ModId = "RelicTracker";
    ...
    private static void Detect()
    {
        try
        {
            ...
            if (_available)
                Log.Info("[RelicTracker] ModConfig detected");
            else
                Log.Info("[RelicTracker] ModConfig not found, using defaults");
        }
        catch
        {
            _available = false;
    ...
    private static void Register()
    {
        ...
        try
        {
            ...
            StartWatchingUI();
            Log.Info("[RelicTracker] ModConfig entries registered");
        }
        catch (Exception e)
        {
            Log.Warn($"[RelicTracker] ModConfig registration failed: {e}");
        }
    }
    ...
    private const string EntriesNodeName = "Entries_RelicTracker";
